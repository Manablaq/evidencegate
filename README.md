...
> Status: current fixed compact v1 candidate implemented and locally release-verified: Direct Mode re-certified 28/28, the exact committed clean-tree supported-runtime finality run passed with five validators, and the reviewer-style audit passed. The current fixed source has not been deployed.
